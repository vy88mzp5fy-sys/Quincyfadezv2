...
function Stepper({ value, suffix = "", min = 0, max = 999, step = 1, onChange, disabled }) {
  ...
  return (
    <View style={styles.stepper}>
      <Pressable disabled={disabled || Number(value || 0) <= min} onPress={() => change(-step)} style={styles.stepButton}><Text style={styles.stepText}>−</Text></Pressable>
      <Text style={styles.stepValue}>{value}{suffix}</Text>
      <Pressable disabled={disabled || Number(value || 0) >= max} onPress={() => change(step)} style={styles.stepButton}><Text style={styles.stepText}>＋</Text></Pressable>
    </View>
  );
}
...
function SettingRow({ title, meta, control, last = false }) {
  return (
    <View style={[styles.settingRow, !last && styles.settingBorder]}>
      <View style={styles.settingCopy}><Text style={styles.settingName}>{title}</Text><Text style={styles.settingMeta}>{meta}</Text></View>
      {control}
    </View>
  );
}
...
export default function AdminScreen({ onExit }) {
  ...
  const saveSettings = async (patch) => {
    ...
    try {
      ...
      setSettings(data.settings || optimistic); setSavedMessage("Saved");
  ...
  const body = useMemo(() => {
    ...
    if (tab === "schedule") return <>
      <Text style={styles.kicker}>SCHEDULE</Text><Text style={styles.pageTitle}>Control Your Time.</Text><Text style={styles.pageText}>Your next seven days, pulled directly from the QuincyFadez booking system.</Text>
      <View style={styles.segmentRow}>{["DAY", "WEEK", "AGENDA"].map((x, i) => <View key={x} style={[styles.segment, i === 1 && styles.segmentActive]}><Text style={[styles.segmentText, i === 1 && styles.segmentTextActive]}>{x}</Text></View>)}</View>
    ...
    if (tab === "insights") return <>
      <Text style={styles.kicker}>INSIGHTS</Text><Text style={styles.pageTitle}>Know Your Business.</Text><Text style={styles.pageText}>No fake demo numbers. This area only shows data created by real QuincyFadez bookings.</Text>
      <View style={styles.segmentRow}>{["DAY", "WEEK", "MONTH"].map((x, i) => <View key={x} style={[styles.segment, i === 1 && styles.segmentActive]}><Text style={[styles.segmentText, i === 1 && styles.segmentTextActive]}>{x}</Text></View>)}</View>
      <View style={[styles.metricsGrid, { marginTop: 12 }]}><MetricCard label="TODAY'S REVENUE" value={money(overview?.today_revenue)} /><MetricCard label="TODAY'S BOOKINGS" value={String(overview?.today_bookings ?? 0)} /><MetricCard label="NEW CLIENTS" value={String(overview?.new_clients ?? 0)} /><MetricCard label="UTILISATION" value={overview?.utilisation_percent == null ? "—" : `${overview.utilisation_percent}%`} /></View>
    ...
    if (tab === "clients") return <>
      <View style={styles.sectionTopRow}><View><Text style={styles.kicker}>CLIENTS</Text><Text style={styles.pageTitle}>Your Client Book.</Text></View><View style={styles.addCircle}><Text style={styles.addCircleText}>＋</Text></View></View>
      <Text style={styles.pageText}>Search real client profiles, booking history and spend.</Text>
      <View style={styles.searchShell}><Text style={styles.searchIcon}>⌕</Text><TextInput value={clientSearch} onChangeText={setClientSearch} placeholder="Search Name, Phone Or Email" placeholderTextColor="#666" style={styles.searchInput} /></View>
    ...
    return <>
      <View style={styles.settingsHeader}><View><Text style={styles.kicker}>SETTINGS</Text><Text style={styles.pageTitle}>Run QuincyFadez Your Way.</Text></View>{saving ? <ActivityIndicator color={GOLD_LIGHT} /> : savedMessage ? <Text style={styles.savedText}>✓ SAVED</Text> : null}</View>
      <Text style={styles.pageText}>Only useful controls for running the business — no filler settings.</Text>

      <SettingsSection title="PROFILE">
        <SettingRow title="Profile" meta="Business details and owner profile" control={<Text style={styles.rowArrow}>›</Text>} />
        <SettingRow title="Services" meta="Pricing, duration and service availability" control={<Text style={styles.rowArrow}>›</Text>} />
        <SettingRow title="Location" meta="Business address and map details" control={<Text style={styles.rowArrow}>›</Text>} />
        <SettingRow last title="Share Booking Link" meta="Copy or share your direct QuincyFadez booking link" control={<Text style={styles.rowArrow}>›</Text>} />
      </SettingsSection>

      <SettingsSection title="BOOKINGS">
        <SettingRow title="Schedule" meta="Working hours, breaks and blocked time" control={<Text style={styles.rowArrow}>›</Text>} />
        <SettingRow title="Slot Frequency" meta="How often appointment start times appear" control={<Stepper value={settings?.slot_interval_minutes ?? 15} suffix="m" min={5} max={60} step={5} disabled={saving} onChange={(value) => saveSettings({ slot_interval_minutes: value })} />} />
        <SettingRow title="Minimum Notice" meta="How close to an appointment clients can book" control={<Stepper value={settings?.minimum_notice_minutes ?? 60} suffix="m" min={0} max={1440} step={15} disabled={saving} onChange={(value) => saveSettings({ minimum_notice_minutes: value })} />} />
        <SettingRow title="Booking Horizon" meta="How far ahead clients can book" control={<Stepper value={settings?.booking_window_days ?? 60} suffix="d" min={7} max={365} step={7} disabled={saving} onChange={(value) => saveSettings({ booking_window_days: value })} />} />
        <SettingRow title="Booking Approval" meta="Require you to approve requests before they become confirmed" control={<Toggle value={Boolean(settings?.booking_approval_required)} disabled={saving} onChange={(value) => saveSettings({ booking_approval_required: value })} />} />
        {settings?.booking_approval_required ? <SettingRow title="Approval Expiry" meta="Automatically expire unanswered booking requests" control={<Stepper value={settings?.booking_approval_expiry_minutes ?? 30} suffix="m" min={5} max={1440} step={5} disabled={saving} onChange={(value) => saveSettings({ booking_approval_expiry_minutes: value })} />} /> : null}
        <SettingRow title="Waiting List" meta="Let clients join a list when suitable times are unavailable" control={<Toggle value={Boolean(settings?.waiting_list_enabled)} disabled={saving} onChange={(value) => saveSettings({ waiting_list_enabled: value })} />} />
        <SettingRow title="Cancellation Notice" meta="Minimum notice clients need to cancel in-app" control={<Stepper value={settings?.cancellation_notice_hours ?? 12} suffix="h" min={0} max={168} step={1} disabled={saving} onChange={(value) => saveSettings({ cancellation_notice_hours: value })} />} />
        <SettingRow last title="Reschedule Notice" meta="Minimum notice clients need to move a booking" control={<Stepper value={settings?.reschedule_notice_hours ?? 12} suffix="h" min={0} max={168} step={1} disabled={saving} onChange={(value) => saveSettings({ reschedule_notice_hours: value })} />} />
      </SettingsSection>

      <SettingsSection title="AUTOMATIONS">
        <SettingRow title="Booking Confirmed" meta="Automatic confirmation after a successful booking" control={<Toggle value={Boolean(automation.booking_confirmed?.enabled)} disabled={saving} onChange={(value) => updateAutomation("booking_confirmed", { enabled: value })} />} />
        <SettingRow title="Booking Reminder" meta={`${automation.booking_reminder?.timing_hours ?? 24} hours before the appointment`} control={<Toggle value={Boolean(automation.booking_reminder?.enabled)} disabled={saving} onChange={(value) => updateAutomation("booking_reminder", { enabled: value })} />} />
        {automation.booking_reminder?.enabled ? <SettingRow title="Reminder Timing" meta="Choose when the reminder is sent" control={<Stepper value={automation.booking_reminder?.timing_hours ?? 24} suffix="h" min={1} max={168} step={1} disabled={saving} onChange={(value) => updateAutomation("booking_reminder", { timing_hours: value })} />} /> : null}
        <SettingRow title="Rescheduled Booking" meta="Notify the client immediately after a booking moves" control={<Toggle value={Boolean(automation.rescheduled_booking?.enabled)} disabled={saving} onChange={(value) => updateAutomation("rescheduled_booking", { enabled: value })} />} />
        <SettingRow title="Leave A Review" meta="Ask for a review after a completed appointment" control={<Toggle value={Boolean(automation.leave_a_review?.enabled)} disabled={saving} onChange={(value) => updateAutomation("leave_a_review", { enabled: value })} />} />
        <SettingRow title="Waiting List Alerts" meta="Notify waiting clients when a suitable slot opens" control={<Toggle value={Boolean(automation.waiting_list_alert?.enabled)} disabled={saving || !settings?.waiting_list_enabled} onChange={(value) => updateAutomation("waiting_list_alert", { enabled: value })} />} />
        <SettingRow title="Re-book Reminder" meta={`${automation.rebook_reminder?.timing_weeks ?? 3} weeks after the last visit`} control={<Toggle value={Boolean(automation.rebook_reminder?.enabled)} disabled={saving} onChange={(value) => updateAutomation("rebook_reminder", { enabled: value })} />} />
        {automation.rebook_reminder?.enabled ? <SettingRow title="Re-book Timing" meta="Choose between 1 and 6 weeks" control={<Stepper value={automation.rebook_reminder?.timing_weeks ?? 3} suffix="w" min={1} max={6} step={1} disabled={saving} onChange={(value) => updateAutomation("rebook_reminder", { timing_weeks: value })} />} /> : null}
        <SettingRow title="Lapsed Client Win-Back" meta="Optional return-client automation for inactive clients" control={<Toggle value={Boolean(automation.lapsed_client_winback?.enabled)} disabled={saving} onChange={(value) => updateAutomation("lapsed_client_winback", { enabled: value })} />} />
        <SettingRow last title="Google Review Booster" meta="Optional follow-up for clients who have not reviewed yet" control={<Toggle value={Boolean(automation.google_review_booster?.enabled)} disabled={saving} onChange={(value) => updateAutomation("google_review_booster", { enabled: value })} />} />
      </SettingsSection>

      <SettingsSection title="PAYMENTS">
        <SettingRow title="Payment Methods" meta="Stripe and booking protection" control={<Text style={styles.rowArrow}>›</Text>} />
        <SettingRow title="Deposits" meta="Require a deposit before confirming selected bookings" control={<Toggle value={Boolean(settings?.deposits_enabled)} disabled={saving} onChange={(value) => saveSettings({ deposits_enabled: value })} />} />
        {settings?.deposits_enabled ? <SettingRow title="Deposit Amount" meta="Flat deposit amount" control={<Stepper value={settings?.deposit_amount ?? 0} suffix="£" min={0} max={100} step={5} disabled={saving} onChange={(value) => saveSettings({ deposit_amount: value })} />} /> : null}
        <SettingRow title="Cancellation Fee" meta="Enable a configurable cancellation fee" control={<Toggle value={Boolean(settings?.cancellation_fee_enabled)} disabled={saving} onChange={(value) => saveSettings({ cancellation_fee_enabled: value })} />} />
        {settings?.cancellation_fee_enabled ? <SettingRow last title="Cancellation Fee Amount" meta="Flat cancellation fee" control={<Stepper value={settings?.cancellation_fee_amount ?? 0} suffix="£" min={0} max={100} step={5} disabled={saving} onChange={(value) => saveSettings({ cancellation_fee_amount: value })} />} /> : null}
      </SettingsSection>

      <SettingsSection title="BUSINESS & GROWTH">
        {[
          ["Client Subscriptions", "Plans, allowances and member benefits"],
          ["Reviews", "Review prompts and Google review flow"],
          ["Promotions", "Create controlled client offers"],
          ["Referral Programme", "Reward client referrals"],
          ["Digital Business Card", "Shareable QuincyFadez contact card"],
        ].map(([title, meta], index, arr) => <SettingRow key={title} last={index === arr.length - 1} title={title} meta={meta} control={<Text style={styles.rowArrow}>›</Text>} />)}
      </SettingsSection>

      <SettingsSection title="BUSINESS SETTINGS">
        <SettingRow title="Notifications" meta="Master control for business alerts and client automation" control={<Toggle value={Boolean(settings?.notifications_enabled)} disabled={saving} onChange={(value) => saveSettings({ notifications_enabled: value })} />} />
        <SettingRow last title="Policies" meta="Booking, lateness, cancellation and payment policies" control={<Text style={styles.rowArrow}>›</Text>} />
      </SettingsSection>

      <SettingsSection title="ACCOUNT & HELP">
        {[
          ["Account", "Admin security and owner access"],
          ["Help & Support", "Support and diagnostics"],
          ["FAQs", "Common QuincyFadez admin questions"],
          ["Send Feedback", "Send product feedback"],
        ].map(([title, meta]) => <SettingRow key={title} title={title} meta={meta} control={<Text style={styles.rowArrow}>›</Text>} />)}
        <Pressable onPress={logout}><SettingRow last title="Log Out" meta="End this secure admin session" control={<Text style={[styles.rowArrow, { color: "#D98778" }]}>›</Text>} /></Pressable>
      </SettingsSection>
    </>;
  }, [tab, overview, bookings, filteredClients, clientSearch, settings, saving, savedMessage]);
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.shell}>
        <View style={styles.adminHeader}>
          <View><Text style={styles.brand}>QUINCYFADEZ</Text><Text style={styles.adminLabel}>BARBER ADMIN</Text></View>
          <View style={styles.headerActions}><Pressable onPress={logout} style={styles.exitButton}><Text style={styles.exitText}>LOG OUT</Text></Pressable>{onExit ? <Pressable onPress={onExit} style={styles.exitButton}><Text style={styles.exitText}>EXIT</Text></Pressable> : null}</View>
        </View>
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          {loading ? <View style={styles.loadingBar}><ActivityIndicator color={GOLD_LIGHT} size="small" /><Text style={styles.loadingText}>SYNCING LIVE DATA…</Text></View> : null}
          {error ? <Text style={styles.errorText}>{error}</Text> : null}
          {body}
        </ScrollView>
