...
function paymentStatusLabel(value) {
  const status = String(value || "not_charged").replaceAll("_", " ").trim();
  return status ? status.replace(/\b\w/g, (letter) => letter.toUpperCase()) : "Not Charged";
}

function paymentTone(value) {
  const status = String(value || "not_charged").toLowerCase();
  if (["paid", "captured", "succeeded", "partially_paid"].includes(status)) return "positive";
  if (["failed", "payment_failed", "past_due"].includes(status)) return "danger";
  return "neutral";
}

function PaymentPill({ status }) {
  const tone = paymentTone(status);
  return <View style={[styles.paymentPill, tone === "positive" && styles.paymentPillPositive, tone === "danger" && styles.paymentPillDanger]}><Text style={[styles.paymentPillText, tone === "positive" && styles.paymentPillTextPositive, tone === "danger" && styles.paymentPillTextDanger]}>{paymentStatusLabel(status).toUpperCase()}</Text></View>;
}
...
export default function ClientManagementPanel({ client, busy, onUpdate }) {
  ...
  const loadPayments = async () => {
    ...
    setPaymentsError("");
    try {
      const response = await fetch(`${API_URL}/api/booking/appointments/${encodeURIComponent(client.client_key)}`);
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.detail || "Payment information could not be loaded.");
      setPaymentBookings(data.bookings || []);
    } catch (err) {
      setPaymentBookings([]);
      setPaymentsError(err.message || "Payment information is unavailable.");
    } finally {
      setPaymentsLoading(false);
    }
  };

  useEffect(() => { loadAppointments(); loadPayments(); }, [client?.client_key]);

  const paymentSummary = useMemo(() => {
    const ordered = [...paymentBookings].sort((a, b) => String(b.created_at || b.start_at || "").localeCompare(String(a.created_at || a.start_at || "")));
    const totals = ordered.reduce((sum, booking) => {
      sum.captured += Number(booking.captured_amount || 0);
      sum.refunded += Number(booking.refunded_amount || 0);
      sum.deposits += Number(booking.deposit_amount_captured || 0);
      sum.fees += Number(booking.cancellation_fee_captured || 0);
      if (Number(booking.captured_amount || 0) > 0) sum.chargedBookings += 1;
      return sum;
    }, { captured: 0, refunded: 0, deposits: 0, fees: 0, chargedBookings: 0 });
    const latest = ordered.find((booking) => booking.payment_status && booking.payment_status !== "not_charged") || ordered[0] || null;
    return { ...totals, net: Math.max(0, totals.captured - totals.refunded), latest };
  }, [paymentBookings]);
  ...
  const requestBlockChange = () => {
    if (busy) return;
    if (client?.blocked) {
      Alert.alert("Unblock Client?", "This will allow this client to make new online bookings again.", [
    ...
    Alert.alert("Block Online Booking?", "This stops new online bookings and reschedules for this client. Existing appointments are not cancelled automatically.", [
  ...
  const runAppointmentAction = async (booking, status) => {
    ...
    const perform = async () => {
      ...
      try {
        ...
        await Promise.all([loadAppointments(), loadPayments()]);
        await onUpdate({});
      } catch (err) {
        setAppointmentError(err.message);
      } finally {
    ...
    if (status === "completed" || status === "confirmed") { await perform(); return; }
  ...
  return <View style={styles.card}>
    <View style={styles.header}><View style={styles.headerCopy}><Text style={styles.eyebrow}>MANAGEMENT</Text><Text style={styles.title}>Client Controls</Text></View><View style={[styles.statusPill, client?.blocked && styles.statusPillBlocked]}><Text style={[styles.statusText, client?.blocked && styles.statusTextBlocked]}>{client?.blocked ? "BLOCKED" : "ACTIVE"}</Text></View></View>

    <View style={styles.reliabilityCard}><View><Text style={styles.sectionLabelNoMargin}>CLIENT RELIABILITY</Text><Text style={styles.reliabilityValue}>{reliability}</Text></View><View style={styles.reliabilityStats}><Text style={styles.reliabilityStat}>{completed} Completed</Text><Text style={styles.reliabilityStat}>{noShows} No-Shows</Text><Text style={styles.reliabilityStat}>{cancelled} Cancelled</Text></View></View>

    <View style={styles.paymentCard}>
      <View style={styles.paymentHeader}><View><Text style={styles.sectionLabelNoMargin}>PAYMENT SNAPSHOT</Text><Text style={styles.paymentTitle}>Real Money Only</Text></View><Pressable disabled={paymentsLoading} onPress={loadPayments} style={styles.refreshButton}><Text style={styles.refreshText}>{paymentsLoading ? "…" : "REFRESH"}</Text></Pressable></View>
      <Text style={styles.paymentHelp}>Captured and refunded totals come from booking payment state, not appointment prices or saved cards.</Text>
      {paymentsError ? <Text style={styles.errorText}>{paymentsError}</Text> : null}
      <View style={styles.paymentMetrics}><View style={styles.paymentMetric}><Text style={styles.paymentMetricLabel}>NET CAPTURED</Text><Text style={styles.paymentMetricValue}>{money(paymentSummary.net)}</Text></View><View style={styles.paymentMetric}><Text style={styles.paymentMetricLabel}>REFUNDED</Text><Text style={styles.paymentMetricValue}>{money(paymentSummary.refunded)}</Text></View><View style={styles.paymentMetric}><Text style={styles.paymentMetricLabel}>PAID BOOKINGS</Text><Text style={styles.paymentMetricValue}>{paymentSummary.chargedBookings}</Text></View></View>
      <View style={styles.paymentDetailRow}><View><Text style={styles.paymentDetailLabel}>DEPOSITS CAPTURED</Text><Text style={styles.paymentDetailValue}>{money(paymentSummary.deposits)}</Text></View><View><Text style={styles.paymentDetailLabel}>CANCELLATION FEES</Text><Text style={styles.paymentDetailValue}>{money(paymentSummary.fees)}</Text></View></View>
      <View style={styles.latestPayment}><View style={styles.latestPaymentCopy}><Text style={styles.paymentDetailLabel}>LATEST PAYMENT STATE</Text><Text style={styles.latestPaymentMeta}>{paymentSummary.latest ? `${paymentSummary.latest.service || "Booking"} · ${formatDate(paymentSummary.latest.start_at)}` : "No Booking Payment State Yet"}</Text></View><PaymentPill status={paymentSummary.latest?.payment_status || "not_charged"} /></View>
    </View>

    {client?.blocked ? <View style={styles.blockedBanner}><Text style={styles.blockedTitle}>Online Booking Blocked</Text><Text style={styles.blockedText}>{client.block_reason || "No reason saved."}</Text><Text style={styles.blockedMeta}>Existing appointments stay in the schedule unless you cancel them separately.</Text></View> : null}

    <Text style={styles.sectionLabel}>CLIENT TAGS</Text><Text style={styles.help}>Use tags to spot important client context quickly. They are private and only visible in Admin.</Text>
    <View style={styles.tagsWrap}>{QUICK_TAGS.map((tag) => { const active = tagSet.has(tag.toLowerCase()); return <Pressable key={tag} disabled={busy} onPress={() => toggleTag(tag)} style={[styles.tag, active && styles.tagActive]}><Text style={[styles.tagText, active && styles.tagTextActive]}>{active ? "✓ " : ""}{tag}</Text></Pressable>; })}</View>
    {tags.filter((tag) => !QUICK_TAGS.some((quick) => quick.toLowerCase() === tag.toLowerCase())).length ? <View style={styles.customTagsWrap}>{tags.filter((tag) => !QUICK_TAGS.some((quick) => quick.toLowerCase() === tag.toLowerCase())).map((tag) => <Pressable key={tag} onPress={() => toggleTag(tag)} style={[styles.tag, styles.tagActive]}><Text style={[styles.tagText, styles.tagTextActive]}>× {tag}</Text></Pressable>)}</View> : null}
    <View style={styles.customRow}><TextInput value={customTag} onChangeText={setCustomTag} maxLength={28} placeholder="Add Custom Tag" placeholderTextColor="#555" style={styles.input} onSubmitEditing={addCustomTag} returnKeyType="done" /><Pressable disabled={!customTag.trim() || busy} onPress={addCustomTag} style={[styles.addButton, (!customTag.trim() || busy) && styles.disabled]}><Text style={styles.addButtonText}>ADD</Text></Pressable></View>
    ...
    <View style={styles.divider} /><Text style={styles.sectionLabel}>BOOKING ACCESS</Text><Text style={styles.help}>Blocking is for clients you do not want making new bookings through the app. It does not erase their history.</Text>
    ...
    <Pressable disabled={busy} onPress={requestBlockChange} style={[styles.accessButton, client?.blocked ? styles.unblockButton : styles.blockButton, busy && styles.disabled]}><Text style={[styles.accessButtonText, client?.blocked ? styles.unblockText : styles.blockText]}>{busy ? "UPDATING…" : client?.blocked ? "UNBLOCK ONLINE BOOKING" : "BLOCK ONLINE BOOKING"}</Text></Pressable>

    <View style={styles.divider} /><View style={styles.appointmentHeader}><View><Text style={styles.sectionLabelNoMargin}>APPOINTMENT MANAGEMENT</Text><Text style={styles.help}>Manage this client’s upcoming bookings and see the payment state beside each one.</Text></View><Pressable disabled={appointmentsLoading || appointmentBusy} onPress={() => { loadAppointments(); loadPayments(); }} style={styles.refreshButton}><Text style={styles.refreshText}>{appointmentsLoading ? "…" : "REFRESH"}</Text></Pressable></View>
    ...
    {appointments.map((booking) => {
      ...
      return <View key={booking.id} style={styles.appointmentCard}>
        ...
        <View style={styles.bookingPaymentRow}><View><Text style={styles.bookingPaymentLabel}>PAYMENT</Text><Text style={styles.bookingPaymentMeta}>{Number(booking.captured_amount || 0) > 0 ? `${money(booking.captured_amount)} captured${Number(booking.refunded_amount || 0) > 0 ? ` · ${money(booking.refunded_amount)} refunded` : ""}` : booking.payment_method_verified ? "Card Verified · No Charge Yet" : "No Captured Payment"}</Text></View><PaymentPill status={booking.payment_status || "not_charged"} /></View>
        <View style={styles.appointmentActions}>{status === "pending" ? <Pressable disabled={isBusy} onPress={() => runAppointmentAction(booking, "confirmed")} style={[styles.actionPrimary, isBusy && styles.disabled]}><Text style={styles.actionPrimaryText}>{isBusy ? "…" : "APPROVE"}</Text></Pressable> : null}{status === "confirmed" ? <Pressable disabled={isBusy} onPress={() => runAppointmentAction(booking, "completed")} style={[styles.actionPrimary, isBusy && styles.disabled]}><Text style={styles.actionPrimaryText}>{isBusy ? "…" : "COMPLETE"}</Text></Pressable> : null}{status === "confirmed" ? <Pressable disabled={isBusy} onPress={() => runAppointmentAction(booking, "no_show")} style={[styles.actionNeutral, isBusy && styles.disabled]}><Text style={styles.actionNeutralText}>NO-SHOW</Text></Pressable> : null}{(status === "pending" || status === "confirmed") ? <Pressable disabled={isBusy} onPress={() => runAppointmentAction(booking, "cancelled")} style={[styles.actionDanger, isBusy && styles.disabled]}><Text style={styles.actionDangerText}>{status === "pending" ? "DECLINE" : "CANCEL"}</Text></Pressable> : null}</View>
      </View>;
    })}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  paymentCard:{marginTop:12,borderRadius:15,borderWidth:1,borderColor:"#3A3020",backgroundColor:"#0E0B07",padding:13},paymentHeader:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:10},paymentTitle:{color:"#EFE6D7",fontSize:13,fontWeight:"800",marginTop:5},paymentHelp:{color:"#817765",fontSize:7.7,lineHeight:12,marginTop:7},paymentMetrics:{flexDirection:"row",gap:6,marginTop:11},paymentMetric:{flex:1,minHeight:62,borderRadius:11,borderWidth:1,borderColor:"#27231C",backgroundColor:"#090807",padding:9},paymentMetricLabel:{color:"#725F3F",fontSize:5.3,letterSpacing:.7,fontWeight:"900"},paymentMetricValue:{color:"#EFE7D8",fontSize:13,fontWeight:"800",marginTop:7},paymentDetailRow:{flexDirection:"row",justifyContent:"space-between",gap:12,marginTop:10,paddingTop:10,borderTopWidth:1,borderTopColor:"#252019"},paymentDetailLabel:{color:"#75644A",fontSize:5.7,letterSpacing:.7,fontWeight:"900"},paymentDetailValue:{color:"#CFC4B0",fontSize:9,fontWeight:"800",marginTop:4},latestPayment:{marginTop:10,borderRadius:11,borderWidth:1,borderColor:"#29251D",backgroundColor:"#090807",padding:10,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:10},latestPaymentCopy:{flex:1},latestPaymentMeta:{color:"#858585",fontSize:7.2,marginTop:4},paymentPill:{borderRadius:9,borderWidth:1,borderColor:"#3A3020",backgroundColor:"#100D07",paddingHorizontal:7,paddingVertical:5},paymentPillPositive:{borderColor:"#315342",backgroundColor:"#09110D"},paymentPillDanger:{borderColor:"#5D2D27",backgroundColor:"#160B09"},paymentPillText:{color:"#A88A54",fontSize:5.2,letterSpacing:.5,fontWeight:"900"},paymentPillTextPositive:{color:"#91D1AD"},paymentPillTextDanger:{color:"#D98778"},
