...
function Input({ icon, right, ...props }) {
  return <View style={s.inputWrap}><Text style={s.inputIcon}>{icon}</Text><TextInput placeholderTextColor="#817D76" autoCorrect={false} style={s.input} {...props} />{right}</View>;
}
...
export default function MockupAuthScreenV2({ onClient, onAdmin }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(REMEMBERED).then((saved) => { if (saved) setEmail(saved); }).catch(() => {});
  }, []);

  const rememberEmail = async (value) => {
    if (remember) await AsyncStorage.setItem(REMEMBERED, value);
    else await AsyncStorage.removeItem(REMEMBERED);
  };
  ...
  return <Marble><SafeAreaView style={s.safe}><StatusBar barStyle="light-content" /><ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <View style={s.card}>
      ...
      <Input icon="▣" value={password} onChangeText={setPassword} placeholder="••••••••" secureTextEntry={!showPassword} autoCapitalize="none" onSubmitEditing={submit} right={<Pressable onPress={() => setShowPassword((v) => !v)} style={s.eyeButton}><Text style={s.eye}>{showPassword ? "◉" : "◎"}</Text></Pressable>} />
      {!signup ? <View style={s.helperRow}><Pressable onPress={() => setRemember((v) => !v)} style={s.remember}><View style={[s.checkbox, remember && s.checkboxOn]}>{remember ? <Text style={s.check}>✓</Text> : null}</View><Text style={s.helperText}>Remember me</Text></Pressable><Pressable onPress={forgot}><Text style={s.forgot}>Forgot password?</Text></Pressable></View> : null}
...
const s = StyleSheet.create({
  ...
  card: { borderRadius: 20, borderWidth: 1, borderColor: "rgba(214,164,67,.30)", backgroundColor: "rgba(13,13,12,.94)", padding: 20, ...cardShadow },
  ...
  inputWrap: { height: 58, borderRadius: 12, borderWidth: 1, borderColor: "rgba(255,255,255,.14)", backgroundColor: "rgba(5,5,5,.84)", flexDirection: "row", alignItems: "center", paddingHorizontal: 15, marginTop: 12 }, inputIcon: { color: M.muted, fontSize: 20, width: 33 }, input: { flex: 1, color: M.text, fontSize: 14 }, eyeButton: { width: 34, height: 40, alignItems: "center", justifyContent: "center" }, eye: { color: M.muted, fontSize: 19 },
  ...
  error: { borderRadius: 9, borderWidth: 1, borderColor: "#63372E", backgroundColor: M.redBg, padding: 10, marginTop: 12 }, errorText: { color: M.red, fontSize: 10.5, lineHeight: 15 },
