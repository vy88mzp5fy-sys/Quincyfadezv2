...
function Row({icon,title,sub,onPress,tone="normal",last=false}){return <Pressable onPress={onPress} style={({pressed})=>[s.row,!last&&s.rowLine,pressed&&{opacity:.62}]}><View style={[s.rowIcon,tone==="danger"&&s.rowIconDanger]}><Text style={[s.rowGlyph,tone==="danger"&&{color:M.red}]}>{icon}</Text></View><View style={{flex:1}}><Text style={[s.rowTitle,tone==="danger"&&{color:M.red}]}>{title}</Text>{sub?<Text style={s.rowSub}>{sub}</Text>:null}</View><Text style={s.chev}>›</Text></Pressable>}
...
export default function QFProfileScreen({onBack,go,onLogout}){
  ...
  return <Marble><SafeAreaView style={s.safe}><StatusBar barStyle="light-content"/><ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
    ...
    <Section title="ACCOUNT"><Row icon="○" title="Personal information" sub="Name, email and mobile number" onPress={()=>go?.("personal")}/><Row icon="◉" title="Notifications" sub="Booking updates and reminders" onPress={()=>go?.("notifications")}/><Row icon="↻" title="Booking preferences" sub="Your usual service and preferred time" onPress={()=>go?.("preferences")} last/></Section>
    <Section title="SECURITY"><Row icon="◇" title="Change password" sub="Update your account password" onPress={()=>go?.("security")} last/></Section>
    <Section title="SUPPORT"><Row icon="◌" title="WhatsApp QuincyFadez" sub="Get help with a booking" onPress={()=>open(WHATSAPP)}/><Row icon="☆" title="Google reviews" sub="See live client feedback" onPress={()=>go?.("reviews")}/><Row icon="↗" title="Terms & privacy" sub="View QuincyFadez online" onPress={()=>open(SITE)} last/></Section>
...
const s=StyleSheet.create({
  safe:{flex:1},content:{paddingHorizontal:18,paddingTop:4,paddingBottom:120},identity:{padding:15,flexDirection:"row",alignItems:"center",gap:12,marginTop:7},avatar:{width:54,height:54,borderRadius:18,backgroundColor:"rgba(169,184,255,.12)",borderWidth:1,borderColor:"rgba(169,184,255,.20)",alignItems:"center",justifyContent:"center"},avatarText:{color:M.accentSoft,fontSize:15,fontWeight:"900",letterSpacing:.5},name:{color:M.text,fontSize:14.5,fontWeight:"700"},email:{color:M.muted,fontSize:9.5,marginTop:4},phone:{color:M.muted2,fontSize:8.5,marginTop:3},edit:{height:38,borderRadius:12,borderWidth:1,borderColor:"rgba(169,184,255,.18)",backgroundColor:"rgba(169,184,255,.08)",paddingHorizontal:12,alignItems:"center",justifyContent:"center"},editText:{color:M.accentSoft,fontSize:9,fontWeight:"800"},section:{color:M.muted,fontSize:8.5,fontWeight:"900",letterSpacing:1.2,marginTop:23,marginBottom:9},group:{overflow:"hidden"},row:{minHeight:72,paddingHorizontal:13,flexDirection:"row",alignItems:"center",gap:11},rowLine:{borderBottomWidth:1,borderBottomColor:"rgba(255,255,255,.055)"},rowIcon:{width:40,height:40,borderRadius:13,backgroundColor:"rgba(169,184,255,.085)",borderWidth:1,borderColor:"rgba(169,184,255,.12)",alignItems:"center",justifyContent:"center"},rowIconDanger:{backgroundColor:"rgba(255,142,150,.055)",borderColor:"rgba(255,142,150,.12)"},rowGlyph:{color:M.accentSoft,fontSize:15,fontWeight:"800"},rowTitle:{color:M.text,fontSize:12.5,fontWeight:"700"},rowSub:{color:M.muted,fontSize:8.8,lineHeight:13,marginTop:4},chev:{color:M.muted2,fontSize:21},footer:{alignItems:"center",marginTop:28},brandDot:{width:7,height:7,borderRadius:4,backgroundColor:M.accent},footerBrand:{color:M.text2,fontSize:8,fontWeight:"900",letterSpacing:1.8,marginTop:8},footerVersion:{color:M.muted2,fontSize:7.5,marginTop:4}
});
