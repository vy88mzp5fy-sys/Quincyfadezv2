...
export default function ScheduleDiary({ bookings = [], view = "week", startDate, onViewChange, onMove, onToday, renderBooking }) {
  ...
  return <>
    ...
    <View style={styles.navigator}>
      <Pressable onPress={() => onMove(-1)} style={styles.navButton}><Text style={styles.navButtonText}>‹</Text></Pressable>
      <Pressable onPress={onToday} style={styles.navCenter}><Text style={styles.navEyebrow}>{view === "agenda" ? "AGENDA" : view.toUpperCase()}</Text><Text style={styles.navDate}>{rangeTitle(startDate, days)}</Text><Text style={styles.todayHint}>TAP FOR TODAY</Text></Pressable>
      <Pressable onPress={() => onMove(1)} style={styles.navButton}><Text style={styles.navButtonText}>›</Text></Pressable>
    </View>
    ...
    {groups.length ? groups.map(([date, items]) => <View key={date} style={styles.dayGroup}>
      <View style={styles.dayHeader}><View><Text style={styles.dayEyebrow}>{new Date(`${date}T12:00:00`).toLocaleDateString("en-GB", { weekday: "long" }).toUpperCase()}</Text><Text style={styles.dayTitle}>{new Date(`${date}T12:00:00`).toLocaleDateString("en-GB", { day: "numeric", month: "long" })}</Text></View><Text style={styles.dayCount}>{items.length} {items.length === 1 ? "BOOKING" : "BOOKINGS"}</Text></View>
      {items.map((booking) => renderBooking(booking))}
    </View>) : <View style={styles.empty}><Text style={styles.emptyTitle}>No Appointments</Text><Text style={styles.emptyText}>There are no confirmed diary records in this selected period.</Text></View>}
  </>;
}
...
const styles = StyleSheet.create({
  ...
  dayGroup:{marginTop:13,borderRadius:19,borderWidth:1,borderColor:"#29251D",backgroundColor:"#0B0A08",padding:14},dayHeader:{flexDirection:"row",alignItems:"flex-end",justifyContent:"space-between",paddingBottom:5},dayEyebrow:{color:GOLD,fontSize:6.5,letterSpacing:1.25,fontWeight:"900"},dayTitle:{color:"#EDEDED",fontSize:15,fontWeight:"750",marginTop:4},dayCount:{color:"#686868",fontSize:6,letterSpacing:.7,fontWeight:"800"},
  empty:{marginTop:14,borderRadius:19,borderWidth:1,borderColor:BORDER,backgroundColor:PANEL,paddingVertical:30,paddingHorizontal:20,alignItems:"center"},emptyTitle:{color:"#EDEDED",fontSize:17,fontWeight:"750"},emptyText:{color:"#808080",fontSize:9,lineHeight:15,textAlign:"center",marginTop:7},
});